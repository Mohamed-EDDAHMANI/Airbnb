...
<body class="bg-gray-50 min-h-screen flex items-center justify-center px-4 py-8">
    <div class="w-full max-w-2xl bg-white shadow-2xl rounded-2xl p-8">
        ...
        <form action="/singUp" id="signupForm" class="space-y-6" method="POST" enctype="multipart/form-data">
            <!-- Role Selection -->
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-4">Select Your Role</h3>
                <div class="grid grid-cols-2 gap-6">
                    <label class="cursor-pointer">
                        <input type="radio" name="role" value="voyageur" class="hidden" checked>
                        <div class="role-card border-2 border-gray-300 rounded-xl p-6">
                            <div class="text-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-500"
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="role" value="proprietaire" class="hidden">
                        <div class="role-card border-2 border-gray-300 rounded-xl p-6">
                            <div class="text-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-500"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center space-x-6">
                    <div class="shrink-0">
                        <img id="profileImagePreview" class="h-24 w-24 object-cover rounded-full" src=""
                            alt="Profile preview" style="display:none;" />
                    </div>
                    <label class="block">
                        <span class="sr-only">Choose profile photo</span>
                        <input type="file" name="pic" id="profileImageUpload" accept="image/*" class="block w-full text-sm text-slate-500
                    ...
                    <div class="bg-white p-6 rounded-xl max-w-2xl w-full">
                        ...
                        <div class="flex justify-end space-x-4">
                            ...
                            <button type="button" id="cropImageBtn"
                                class="px-4 py-2 bg-[#FF385C] text-white rounded-lg hover:bg-[#E31C5F]">Crop</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" required name="firstName" class="w-full px-4 py-2 border border-gray-300 rounded-lg 
                        focus:ring-2 focus:ring-[#FF385C] focus:border-transparent" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Last Name</label>
                    <input type="text" required name="lastName" class="w-full px-4 py-2 border border-gray-300 rounded-lg 
                        focus:ring-2 focus:ring-[#FF385C] focus:border-transparent" />
                </div>
            </div>
            ...
            <!-- Submit Button -->
            <div class="pt-4">
                <button type="submit" class="w-full py-3 bg-[#FF385C] text-white rounded-lg 
            ...
            <div class="space-y-3">
                <a href="<?= $data ?>" class="w-full flex justify-center py-3 bg-[#4285F4] text-white rounded-lg hover:bg-[#3574e0] transition duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#4285F4] focus:ring-opacity-50">
                    Sign Up with Google
                </a>
                <button type="button" class="w-full py-3 bg-[#1877F2] text-white rounded-lg hover:bg-[#1565d8] transition duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#1877F2] focus:ring-opacity-50">
    ...
    <script>
        ...
        const radioInputs = document.querySelectorAll('input[type="radio"][name="role"]');
        ...
        radioInputs.forEach(input => {
            input.addEventListener('change', () => {
                ...
                // Add selected class to the parent card of checked input
                if (input.checked) {
                    input.closest('label').querySelector('.role-card').classList.add('selected');
                }
            });
        });
